...
std::pair<std::vector<CryptoPP::Integer>, bool> VoterClient::DoVerify() {
  ...
  // std::cout << "[Debug] getting all_partial_dec ... " << std::endl; 
  std::vector<PartialDecryptionRow> all_partial_dec = this->db_driver->all_partial_decryptions();
  // std::cout << "[Debug] Number of all partial_dec in verify -> " << all_partial_dec.size() << std::endl; 
  ...
  // std::cout << "[Debug] check all partial decryptions in the map" << std::endl;
  CryptoPP::Integer pki;
  ...
  // std::cout << "[Debug] combine results" << std::endl;
  for (auto it = partial_dec_map.begin(); it != partial_dec_map.end(); ++it){
